...
- **꺼진 채로 시작** — 처음엔 아무것도 안 하다가 화면을 보고 필요한 구간만 켜서 돌릴 수 있습니다
...
모든 스텝에는 **`이 스텝 사용`**(아예 쓸지 말지)과 **`시작할 때 꺼둠`**(꺼진 채로 시작해서 스위치가 켜야 실행)이 있습니다.
...
| **끄기** | 적은 스텝들을 끕니다 |
| **켜기** | 적은 스텝들을 켭니다 |
| **반대로** | 켜져 있으면 끄고, 꺼져 있으면 켭니다 |
| **만 켜기 (나머지 끄기)** | 적은 스텝들만 켜고 **나머지는 전부 끕니다**. 스위치 스텝은 건드리지 않습니다 |
...
> 다음에 다시 실행하면 원래 상태에서 시작합니다.

### 꺼진 채로 시작하기

각 스텝의 **`시작할 때 꺼둠`** 을 켜두면, 실행을 시작할 때 그 스텝이 꺼진 채로 출발합니다.
스위치가 켜주기 전까지는 건너뜁니다.

이걸 쓰면 **"처음엔 아무것도 안 하다가, 화면을 보고 필요한 것만 켜서 돌리는"** 매크로를 만들 수 있습니다:

1. 스위치 스텝을 뺀 **모든 스텝에 `시작할 때 꺼둠` 을 켭니다**
2. 스위치 스텝은 `만 켜기 (나머지 끄기)` 로 설정합니다

| 스텝 | 설정 |
|---|---|
| 5번 | `상황A.png` 이 보이면 → `1,2,3,4` **만 켜기** |
| 6번 | `상황B.png` 이 보이면 → `1,2,3,4,7,8,9` **만 켜기** |

- 시작하면 1~4·7~9가 전부 꺼져 있어 아무것도 실행되지 않습니다. 스위치만 화면을 지켜봅니다
- `상황A.png` 이 뜨면 1·2·3·4만 켜져서 그것만 돕니다
- `상황B.png` 이 뜨면 7·8·9까지 켜져서 전부 돕니다

> **`이 스텝 사용` 과 다릅니다.** `이 스텝 사용` 을 끄면 그 스텝은 아예 실행되지 않습니다.
> `시작할 때 꺼둠` 은 "지금은 꺼두지만 스위치가 켜면 실행된다" 는 뜻입니다.

> 스위치 스텝까지 `시작할 때 꺼둠` 을 켜면 아무것도 켜줄 수 없으므로,
> 실행할 때 경고를 띄우고 시작하지 않습니다.
